  ...
  return (
    <header className="sticky top-0 z-40 bg-white/80 dark:bg-stone-950/80 backdrop-blur-md border-b border-rose-100 dark:border-stone-800 transition-colors">
      <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
        <Link
          to={user ? "/dashboard" : "/"}
          className="flex items-center gap-2 font-display font-bold text-xl text-stone-800 dark:text-stone-100">
          <Logo size={32} />
          NextRound
        </Link>
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          {user ? (
            <nav className="flex items-center gap-2 sm:gap-4">
              <Link
                to="/dashboard"
                className="text-sm font-medium text-stone-600 dark:text-stone-300 hover:text-rose-500 dark:hover:text-rose-400 transition px-2 py-1">
                Dashboard
              </Link>
              <Link
                to="/setup"
